/**
 * Minigolf Prompt Builder V2 - Optimized for Ollama/LocalLLM
 * Includes calculator baseline and style-specific power guidance
 */
export class MinigolfPromptBuilderV2 {
    constructor(personality) {
        this.personality = personality;
    }

    /**
     * Build minigolf prompt with baseline and power guidance
    ...
    buildPrompt(context) {
        ...
        // Game context
        const gameContext = `Distance to hole: ${state.distanceToHole?.toFixed(1) || '?'} | Strokes: ${state.strokes || 0}`;

        return `MINIGOLF DECISION - RETURN ONLY JSON, NO EXPLANATION
Ball: (${ball.x?.toFixed(1) || '?'}, ${ball.z?.toFixed(1) || '?'})
Target: (${state.hole?.x?.toFixed(1) || '?'}, ${state.hole?.z?.toFixed(1) || '?'})
...
COMMENTARY: Keep it humorous and MAX 15 WORDS. Must reflect your ${style} personality (e.g., aggressive=cocky, chaotic=unhinged, cautious=worried, perfectionist=confident, strategic=calculating).
...
{"angle": NUMBER_HERE, "power": NUMBER_HERE, "commentary": "humorous one-liner max 15 words"}`;
    ...
    _getBehaviorGuide(style) {
        const guides = {
            aggressive: {
                behaviorGuide: 'Go hard with extreme angles and high power.',
                powerRange: '80-120 (aggressive minigolf)'
            },
            chaotic: {
                behaviorGuide: 'Random power, crazy angles, no plan.',
                powerRange: '0-120 (anything goes)'
            },
            cautious: {
                behaviorGuide: 'Safe power, conservative angles, minimal risk.',
                powerRange: '20-50 (safe shots)'
            },
            perfectionist: {
                behaviorGuide: 'Optimize angle and power for hole-in-one.',
                powerRange: '40-80 (precise)'
            },
            strategic: {
                behaviorGuide: 'Balanced approach, think around obstacles.',
                powerRange: '40-80 (balanced)'
            }
        };
...
export default MinigolfPromptBuilderV2;
